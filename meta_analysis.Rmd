...
```{r acf-pacf-returns-meta}
# ACF & PACF of returns_meta
tsdisplay(log(Meta))
```

First, ACF shows gradual decay and PACF clear cuts off at lag 1. 
...
Using matrix to calculate and choose the min of AIC, the result is ARIMA(4,1,5) with drift.
...
The goal of this part is to ensure that the residuals are white Gaussian noise. After getting ARIMA(1,1,0) from auto.arima, we evaluate the residuals of this model. 

```{r residual-check-auto-arima}
# Use sarima from package astsa because many plots shown
# Check residuals ARIMA(1,1,0)
sarima(log(Meta), 1,1,0, Model = FALSE)

## Check residuals ARIMA(4,1,5)
sarima(log(Meta), 4,1,5, Model = FALSE)
```
...
- Q-statistic p-values: if most points are above the blue lines, we can assume white noise. If this is not the case, there might be some correlation left in the residuals, we might consider to fit another model or add a parameter. Here, the bottom plot of ARIMA(1,1,0) displays some points above the line but many are touching or under the line. In contrast, the Lijung-Box for ARIMA(4,1,5) shows all the points above the line.   
