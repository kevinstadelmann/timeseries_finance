...
```{r auto-arima}
# Fit an auto-arima
fit_auto_ari <- auto.arima(y = log(Meta), ic = "aic", trace = TRUE)
fit_auto_ari
...
- AIC and AICc of ARIMA(4,1,4) are smaller than ARIMA(0,1,1). In contrast, BIC of ARIMA(0,1,1) is smaller than that of ARIMA(4,1,4). (i.e. -4.99 < -4.98)

### Validation by fit inspection

```{r actual-resid-plot}
plot(log(d_meta$price_meta))
lines(log(d_meta$price_meta) - resid(fit_arima), col = "blue")
...
```{r train-test-split}
train <- window(ts_meta, end=c(2021,180))
test <- window(ts_meta, start=c(2021, 150))
```
...
pred <- forecast(fit_arima, level = 0.95, h = 60)
autoplot(pred, ylab="Meta")
```
...
- MAPE of ARIMA(4,1,4) is slightly smaller than MAPE of ARIMA(0,1,1)
...
For Meta, skew < 1 indicating a negatively skewed distribution. Degree of freedom shape is around 4, indicating fat tails. 
...
```{r}
# How to get MAPE for the flexgarchfit model???
```
