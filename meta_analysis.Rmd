...
fit_arima <- Arima(y=log(Meta), order=c(ar,1,ma), include.constant = TRUE)
```
...
```{r auto-arima}
# Fit an auto-arima
fit_auto_ari <- auto.arima(y = log(d_meta$price_meta), ic = c("aicc", "aic", "bic"))
fit_auto_ari
```
...
The goal of this part is to ensure that the residuals are white Gaussian noise. After getting ARIMA(0,1,1) from auto.arima, we evaluate the residuals of this model. 
...
- Q-statistic p-values: if most points are above the blue lines, we can assume white noise. If this is not the case, there might be some correlation left in the residuals, we might consider to fit another model or add a parameter. Here, the bottom plot of ARIMA(1,1,0) displays two points above the line but the rest are touching or under the line. In contrast, the Lijung-Box for ARIMA(4,1,5) shows all the points above the line.
- AIC and AICc of ARIMA(4,1,4) are smaller than ARIMA(1,1,0). In contrast, BIC of ARIMA(1,1,0) is smaller than that of ARIMA(4,1,4). (i.e. -4.99 < -4.98)

### Forecasting

```{r forecast-auto.arima}
# Creating test and train dataset
d_train <- window(ts_meta, end = c(2021, 150))
d_test <- window(ts_meta, end = c(2021, 180))

# Forecasting 30 days using astsa package 
sarima.for(d_train, n.ahead = 30, 0, 1, 1)
lines(d_test)

sarima.for(d_train, n.ahead = 30, 4, 1, 4)
lines(d_test)

# Forecasting 30 days using forecast
pred <- forecast(fit_auto_ari, level = 0.95, h = 30)
plot(pred, ylab="Meta")
```

### Measuring model accuracy

```{r accuracy-auto-arima-measurement}
# Check RMSE and MAPE of the fit_auto_ari
accuracy(fit_auto_ari)

# Check RMSE and MAPE of the fit_arima
accuracy(fit_arima)
```

Observations:

- MAPE of ARIMA(4,1,4) < MAPE of ARIMA(0,1,1) but slightly different
- 
