...
# Forecasting 29 days using astsa package 
sarima.for(log(window1_meta), n.ahead = 100, 4, 1, 4)
lines(log(window2_meta))

# Forecasting 29 days using forecast
pred_arima <- forecast(fit_arima, level = 0.95, h = 29)
autoplot(pred_arima, ylab="Meta")
```

### Measuring model accuracy

To measure the model accuracy, we check the Root Mean Square Error (RMSE) and the (MAPE)
...
- Train model forecast on the test data: MAPE = 0.346, RMSE = 0.025
- Test data: MAPE = 0.221, RMSE = 0.0173
- Train data: MAPE = 0.261, RMSE = 0.0196
- The whole time series: MAPE = 0.261, RMSE = 0.0196
- Overall, the  model forecast for the test data have around 34% of error on average prediction compared to the true value. 
...
meta_discrete_returns <- CalculateReturns(Meta, method = "log")
...
train_garch <- meta_discrete_returns[1:(length(meta_discrete_returns) - 100)]

# Test data from day 80 to day 180 2021
test_garch <- meta_discrete_returns[(length(meta_discrete_returns) - 99):length(meta_discrete_returns)]
```
...
garchspec <- ugarchspec(mean.model = list(armaOrder = c(4,4)),
...
# Estimate the model
mean_ar1_garchfit <- ugarchfit(data = train_garch, spec = mean_ar1_garchspec)
...
Here, we fit two models, one simple (standard GARCH (1,1) model with Student $t$ innovations) and one complex (AR(1) GJR GARCH model with skewed Student $t$ innovations) and compare their mean squared prediction errors (MSE), likelihood and information criteria. Higher Likelihood can lead to the risk of over-fitting. However, lower information criteria is better.
...
length(coef(flexgarchfit)) # we now have 14 parameters
```
...
* Sample mean of standardized returns = -0.0075 $\approx$ 0 -> good
...
* Ljung box test: Ho: autocorrelations in the absolute standardized returns are 0. We went to have 0 to have a good model. P-value = 0.4 > 0.05. The model is valid. -> good
...
```{r forecasting-GARCH}
# Way 1: Using urgarchforecast
forecast1 <- ugarchforecast(fitORspec = flexgarchfit, n.ahead = 100)

# For comparison, we make the forecasts from way 1 as vector
f1 <- as.vector(forecast1@forecast$seriesFor)
f1_sigma <- as.vector(forecast1@forecast$sigmaFor)

# Way 2: Boostrap forecast
forecast2 <- ugarchboot(flexgarchfit, method=c("Partial","Full")[1],
                        n.ahead = 100, n.bootpred = 1000, n.bootfit = 1000)

# For comparison, we make the forecasts from way 2 as vector
f2 <- as.vector(forecast2@forc@forecast$seriesFor)
f2_sigma <- as.vector(forecast2@forc@forecast$sigmaFor)
```
...
mean(abs((test_garch - forecast1@forecast$seriesFor) / test_garch))

## Way 2: by accuracy function: RMSE, MAPE for the series
accuracy(f1, test_garch)
accuracy(f2, test_garch)

# RMSE, MAPE for the volatility
accuracy(f1_sigma, sd(test_garch))
accuracy(f2_sigma, sd(test_garch))
```
...
* The MAPE for the returns is extremely high, meaning that the predicted values are much larger than the actual values. Predicting returns for financial time series is very hard. 
* Compared to ARIMA, GARCH(1,1) with ARMA(4,4) performs worse.
...
garch23_fit_total <- ugarchfit(spec = garch23_spec, data = meta_discrete_returns,
                        solver.control = list(trace = 1))

# Fit the train test
garch23_fit_train <- ugarchfit(spec = garch23_spec, data = train_garch,
                        solver.control = list(trace = 1))

# Fit the train test
garch23_fit_test <- ugarchfit(spec = garch23_spec, data = test_garch,
                        solver.control = list(trace = 1))
```

## Prophet

Prophet is an open source library published by Facebook that is based on decomposable (trend+seasonality+holidays) models. We use a decomposable time series model with three main model components: trend, seasonality, and holidays. They are combined in the following equation:
...
```{r}
# Create a new column with logged price
d_meta_prophet <- d_meta %>% 
  mutate(price_meta = log(as.float(Meta))) %>% 
  rename(ds = date_meta,
         y = price_meta)

# 
prophet_model1 <- prophet(d_meta_prophet)
